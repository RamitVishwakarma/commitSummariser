...
export function HeroSection(): React.JSX.Element {
  return (
    <section className="mx-auto flex max-w-[1280px] flex-col items-center px-6 pt-24 pb-32 text-center">
      <motion.div {...PILL_ANIM}>
        <span className="pill text-accent-teal mb-6 inline-flex" style={PILL_BORDER_STYLE}>
          <Sparkles size={12} /> For developers who ship
        </span>
      </motion.div>
      <motion.h1
        {...HEADLINE_ANIM}
        className="max-w-[800px] text-[clamp(36px,5vw,64px)] leading-[1.1] font-bold"
      >
        Your commits tell a story. <span className="text-accent-violet">GitPulse</span> reads it.
      </motion.h1>
      <motion.p
        {...SUBTEXT_ANIM}
        className="text-text-secondary mt-6 max-w-[600px] text-lg leading-relaxed"
      >
        Automatic daily commit tracking, AI-powered weekly and monthly summaries, and a yearly map
        of your growth — all on autopilot.
      </motion.p>
      <motion.div {...BTNS_ANIM} className="mt-10 flex items-center gap-4">
        <Link href="/dashboard" className="btn-primary !px-8 !py-[14px] text-base">
          Get Started
        </Link>
      ...
      <motion.div
        {...MOCK_ANIM}
        className="glass-card mt-20 w-full max-w-[900px] [transform:perspective(1000px)_rotateX(2deg)] overflow-hidden p-6"
      >
        <div className="mb-4 grid grid-cols-4 gap-3">
          {MOCK_STATS.map((s, i) => (
            <div key={s} className="glass-card p-3 text-center">
              <div className="text-text-muted text-[10px]">{MOCK_STAT_LABELS[i]}</div>
        ...
        <div className="flex gap-3">
          <div className="glass-card flex-1 p-4">
            ...
            {MOCK_COMMITS.map((m, i) => (
              <div
                key={m}
                className="flex items-center gap-2 border-b border-[rgba(255,255,255,0.04)] py-1.5"
              >
                <span className="mono text-accent-violet text-[11px]">a3f7b{i}c</span>
          ...
          <div className="glass-card w-[240px] p-4">
            ...
            <div className="mb-2 flex items-center gap-1">
              ...
              <span className="text-accent-teal text-[11px]">AI Generated</span>
            </div>
